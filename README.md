...
#Options for running the application
  ...
  --batchRows BATCHROWS
                        This defines the number of rows to batch before
...
#Basic SQL
  /* */ is the syntax for block comments. -- will specify a comment to the end of the line. 
  
  The create view syntax is defined [here](http://www.postgresql.org/docs/9.1/static/sql-createview.html).
Views are tables which are generated from select statements. The most basic view (called View_Name) is
created by running:
...
able to select specific table columns to generate a new "table" (view). 
  ...
  The select statement can be quite complicated. It is described in detail 
[here](http://www.postgresql.org/docs/9.1/static/sql-select.html). When creating a view from a select
statement, the select statement must be deterministic. This means that no variables are allowed in the
select statement which generates the view as this is getting into scripting. Also, the `SELECT SUM()`
...
sort of handled by a `CASE WHEN ... THEN ... (ELSE) ... END` statement described
[here](http://www.postgresql.org/docs/9.1/static/functions-conditional.html). 
